<?php
session_start();
// Using absolute pathing from current script directory to avoid broken relative hops
...
$game_name = isset($_POST['game_name']) ? trim($_POST['game_name']) : 'memory_game';
...
$total_stages = isset($_POST['total_stages']) ? intval($_POST['total_stages']) : 3;
...
if ($progress_percent <= 0 && $score !== null) {
    $progress_percent = $score;
}

if ($stage_number <= 0) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'Invalid stage_number']);
    exit;
}
...
try {
    // Look up an existing module from your database to fulfill your database's Foreign Key constraint
    ...
    // Check if progress row already exists for this specific stage
    ...
    if ($existing) {
        ...
        // Insert brand new entry with module_id included to pass the foreign key requirement
    ...
    // Check if overall game stages are finished
    if ($is_completed === 1 && $total_stages > 0) {
        $stmtCount = $conn->prepare('SELECT COUNT(*) AS completed_count FROM progress WHERE user_id = ? AND game_name = ? AND stage_number > 0 AND is_completed = 1');
        $stmtCount->execute([$user_id, $game_name]);
        $countRow = $stmtCount->fetch(PDO::FETCH_ASSOC);
        
        $completed_count = intval($countRow['completed_count'] ?? 0);
        ...
        if ($completed_count >= $total_stages) {
            ...
            // Check if summary entry (stage 0) exists
    ...
    // Send valid structural JSON directly back to your front-end JS handler
    echo json_encode([
        'success' => true,
